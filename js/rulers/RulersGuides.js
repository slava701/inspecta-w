...
*/
var RulersGuides = function (evt, dragdrop, windowHeight, windowWidth) {
    'use strict';
    const resizeObserver = new ResizeObserver(entries => {
        for (let entry of entries) {
            if (entry.target === document.documentElement) {
                // Update ruler dimensions when document size changes
                if (rulerStatus === 1) {
                    const scrollSize = getScrollSize();
                    if (wrapper) {
                        wrapper.style.height = scrollSize[1] + 'px';
                        wrapper.style.width = scrollSize[0] + 'px';
                    }
                    if (vRuler) {
                        vRuler.style.height = scrollSize[1] + 'px';
                    }
                    if (hRuler) {
                        hRuler.style.width = scrollSize[0] + 'px';
                    }
                }
            }
        }
    ...
    var doc = document.documentElement,
        ...
        toggleRulers = function () {
            ...
            if (rulerStatus === 1) {
                // Set correct dimensions when enabling rulers
                const scrollSize = getScrollSize();
                if (wrapper) {
                    wrapper.style.height = scrollSize[1] + 'px';
                    wrapper.style.width = scrollSize[0] + 'px';
    ...
    this.disable = function () {
        if (vRuler !== null) {
            //deleteGuides(); // Remove all guides
    ...
    this.enable = function () {
        if (vRuler !== null) {
            // Update dimensions when enabling rulers
            const scrollSize = getScrollSize();
            if (wrapper) {
                wrapper.style.height = scrollSize[1] + 'px';
                wrapper.style.width = scrollSize[0] + 'px';
    ...
    evt.attach('mousedown', document, function (e, src) {
        ...
        if (src.className.indexOf && src.className.indexOf('menu-btn') === -1) {
            menu.close();
        }
        ...
        // Only create guide if we're in the ruler areas
        if (
            (
                (x > vBound && y < hBound) ||
                (y > hBound && x < vBound)
            ...
            // Create guide element but don't position it yet
            guide = document.createElement('div');
            guide.classList.add('selected-guide');
            guide.setAttribute('tabindex', 100 + guidesCnt);

            // Handle guide selection
            guide.addEventListener('mousedown', function (e) {
                // Remove selected class from all guides
                document.querySelectorAll('.guide.selected-guide').forEach(g => {
                    g.classList.remove('selected-guide');
                });
                // Add selected class to clicked guide
                this.classList.add('selected-guide');
                this.focus();

                // Set up drag handling for selected guide
                dragdrop.set(this, {
                    mode: this.type === 'h' ? 2 : 1,
                    ...
                    onmove: function (elem) {
                        ...
                        // Check if guide hits the rulers, accounting for scroll position
                        if (elem.mode === 1 && pos <= vBound) { // Vertical guide hits left ruler
                            elem.remove();
                            delete guides[elem.id];
                            guidesCnt = guidesCnt - 1;
                            if (detailsStatus === 1) {
                                showDetailedInfo();
                            }
                            return;
                        } else if (elem.mode === 2 && pos <= hBound + currentScrollPos[0]) { // Horizontal guide hits top ruler
                            elem.remove();
                            delete guides[elem.id];
                            guidesCnt = guidesCnt - 1;
                            if (detailsStatus === 1) {
            ...
            // Handle drag start separately
            guide.addEventListener('mousemove', function (e) {
                if (this.classList.contains('selected-guide') && e.buttons === 1) {
                    // Only start drag if guide is selected and left mouse button is pressed
                    dragdrop.start(e, this);
                }
            });

            // Handle focus out
            guide.addEventListener("focusout", function () {
                // Don't remove selected class on focus out
                // This allows the guide to stay selected for deletion
            });
            ...
            // Set up initial drag handling for new guide
            dragdrop.set(guide, {
                mode: mode,
                onstart: function (elem) {
                    var text = (elem.mode === 1)
            ...
            // Only start drag if mouse has moved
            var mouseMoveHandler = function (moveEvent) {
                if (!isDragging && (Math.abs(moveEvent.clientX - startX) > 5 || Math.abs(moveEvent.clientY - startY) > 5)) {
                    isDragging = true;
                    // Mouse has moved enough to start drag
            ...
            // Remove handler if mouse is released without dragging
            var mouseUpHandler = function () {
                if (!isDragging) {
                    // If we didn't drag, remove the guide
                    guide.remove();
    ...
    evt.attach('keyup', document, function (e) {
        // Handle backspace/delete for selected guides
        if ((e.key === 'Delete' || e.key === 'Backspace')) {
            const selectedGuide = document.querySelector('.guide.selected-guide');
            if (selectedGuide) {
                console.log('Deleting guide:', selectedGuide.id); // Debug log
                selectedGuide.remove();
                delete guides[selectedGuide.id];
                guidesCnt = guidesCnt - 1;
                if (detailsStatus === 1) {
                    showDetailedInfo();
                }
